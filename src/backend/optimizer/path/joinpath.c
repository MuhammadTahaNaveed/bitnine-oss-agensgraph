					 ...
					 SpecialJoinInfo *sjinfo, List *restrictlist)
{
	...
	foreach(lc, root->join_info_list)
	{
		SpecialJoinInfo *_sjinfo = (SpecialJoinInfo *) lfirst(lc);

		if (bms_overlap(joinrel->relids, _sjinfo->min_righthand) &&
			!bms_overlap(joinrel->relids, _sjinfo->min_lefthand))
			extra.param_source_rels = bms_join(extra.param_source_rels,
											   bms_difference(root->all_baserels,
															  _sjinfo->min_righthand));
	}
